ActiveAdmin.register Order do
  ...
  filter :service_id, label: '服务号', as: :select, collection: Service.where(id: Order.pluck(:service_id).uniq).pluck(:id, 'profiles.name')
  filter :coach_id, label: '私教', as: :select, collection: Coach.where(id: Order.pluck(:coach_id).uniq).pluck(:id, 'profiles.name')
  actions :index, :show
