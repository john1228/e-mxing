module FindManager
  ...
  def courses
    ...
    case sort_info[0]
      ...
      when 'distance'
        sql = "select #{select_field}, st_distance(address_coordinates.lonlat, 'POINT(#{params[:lng]} #{params[:lat]})') distance from address_coordinates,courses,profiles where #{filter} and st_dwithin(places.lonlat, 'POINT(#{params[:lng]} #{params[:lat]})',150000) and  address_coordinates.address_id = ANY(courses.address) order by distance asc limit 25 offset #{((params[:page]||1).to_i - 1)}"
      when 'sale'
