class UsersController < ApiController
  ...
  def sns
    ...
    if login_user.nil?
      ...
      login_user = User.create(sns: "#{params[:sns_name]}_#{params[:sns_id]}",
                               profile_attributes: {name: params[:name], remote_avatar_url: params[:avatar], gender: params[:gender], birthday: params[:birthday]})
    end
    login_user.update(device: params[:device]) if login_user.blank?
