  ...
  class LessonsController < BaseController
    def index
      render json: Success.new(
                 lessons: MembershipCardLog.joins(:membership_card).checkin.confirm.where(membership_cards: {order_id: @coach.orders.pay.pluck(:id), card_type: 3}).order(created_at: :desc).page(params[:page]||1).map { |log|
                   {
