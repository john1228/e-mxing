class Order < ActiveRecord::Base
  ...
  def backend_task
    ...
    user.wallet.update(coupons: (user.wallet.coupons||[] + coupons.split(',')), bean: (user.wallet.bean + bean), action: WalletLog::ACTIONS['订单取消']) if status.eql?(STATUS[:cancel]) && coupons.present?
  end
end
