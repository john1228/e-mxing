namespace :migration do
  ...
  task :member_yoyo => :environment do
    ...
    csv.each do |row|
      ...
      if user.blank?
        user = User.new(mobile: mobile, password: '12345678', profile_attributes: {name: name})
        user.save
      end
      #会员信息
      member = user.members.find_by(service: service, name: name, mobile: mobile)
      if member.blank?
        member = user.members.new(service: service, name: name, mobile: mobile, remark: '合同编号-'+order_no.to_s+'|'+'原卡号-'+card_no.to_s)
        member.save
      end
      #会员卡信息
      ...
      if membership_card.save
        log = membership_card.logs.buy.new(change_amount: value, pay_amount: pay_amount, pay_type: 'other', seller: seller, remark: '合同编号-'+order_no.to_s+'|'+'原卡号-'+card_no.to_s, created_at: create_date, updated_at: create_date)
        log.save
      end
    end
  end
